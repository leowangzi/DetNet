...
def smooth_l1_loss_from_fb(input, target, beta=1. / 9, size_average=True):
    """
...
class _RPN_FPN(nn.Module):
    ...
    def forward(self, rpn_feature_maps, im_info, gt_boxes, num_boxes):        
        ...
        if self.training:
            ...
            # print("DetNet")
            # print("rpn_bbox_inside_weights")
            # print(rpn_bbox_inside_weights.shape)
            # print("rpn_bbox_outside_weights")
            # print(rpn_bbox_outside_weights.shape)

            #############################
            # a = torch.rand([2,5])
            # print(a.shape)
            # print(a)
            # b = a.unsqueeze(2).expand(2, a.size(1), 4)
            # print(b.shape)
            # print(b)
            #############################
            ...
            # self.rpn_loss_box_ref = F.smooth_l1_loss(rpn_bbox_pred_alls, rpn_bbox_targets)
            # print(self.rpn_loss_box_ref)

            # self.rpn_loss_box = smooth_l1_loss_from_fb(rpn_bbox_pred_alls, rpn_bbox_targets)
            # print(self.rpn_loss_box_ref2)
            # exit(1)
