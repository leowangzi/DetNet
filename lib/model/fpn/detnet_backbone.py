...
__all__ = ['DetNet', 'detnet59', 'detnet101']
...
def detnet101(pretrained = False):
    model = DetNet(Bottleneck, [3, 4, 23, 3, 3])
    return model
...
class detnet(_FPN):
    ...
    def _init_modules(self):
        if self.num_layers == 59:
            detnet = detnet59()
            self.model_path = 'data/pretrained_model/detnet59.pth'
        elif self.num_layers == 101:
            detnet = detnet101()
            self.model_path = 'data/pretrained_model/detnet101.pth'

        if self.pretrained == True:
            print("Loading pretrained weights from %s" % (self.model_path))
            state_dict = torch.load(self.model_path, map_location=lambda storage, loc: storage)
            detnet.load_state_dict({k: v for k, v in state_dict.items() if k in detnet.state_dict()}, strict=False)
