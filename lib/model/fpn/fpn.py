...
class _FPN(nn.Module):
        ...
        if cfg.POOLING_MODE == 'align':
            ...
            for i, l in enumerate(range(2, 6)):
                if (roi_level == l).sum() == 0:
                    continue
                idx_l_ref = (roi_level == l).nonzero().squeeze()
                idx_l = (roi_level == l).nonzero()[:, 0]
                # print(idx_l.shape)
                # print(idx_l)
                # print(idx_l_ref.shape)
                # print(idx_l_ref)
                # exit(1)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        # cls_prob_ref = F.softmax(cls_score, dim=1)
        # print(cls_prob)
        # print(cls_prob_ref)
        # exit(1)
        ...
        if self.training:
            ...
            # print(rois_inside_ws.shape)
            # print(rois_outside_ws.shape)
